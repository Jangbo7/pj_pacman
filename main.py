...
from DQN_utils import encode_state, get_legal_actions_mask, calculate_reward
...
def main(env_name, render=True, episodes=2):
    if env_name == "ALE/Pacman-v5" or env_name == "ALE/MsPacman-v5":
        ...
        # image_rgb = cv2.cvtColor(observation, cv2.COLOR_BGR2RGB)
        frame=0
        former_all_game_info = None
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
            cv2.imwrite(tmp.name, image_bgr)
            # image_bgr[:, :] = np.array([0, 0, 0])
        ...
        while not done:
            ...
            if frame < 65:
                ...
                continue
            former_pill_list = pill_list
            former_superpill_list = superpill_list

            state = encode_state(former_all_game_info, former_pill_list, former_superpill_list)             # 第i帧（第一次进循环应该是第0帧）的信息

            legal_actions_mask = get_legal_actions_mask(former_all_game_info)
            action = dqnAgent.choose_action(state, legal_actions_mask)
            observation, _, terminated, truncated, _ = single_action(env, action, 1)
            ...
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                ...
                # 更新 pill_list 和 superpill_list： 如果pacman吃掉了一个豆子，就从列表中移除
                ...
                # 更新next_pill_list 和 next_superpill_list
                next_pill_list = pill_list
                next_superpill_list = superpill_list
                ...
                reward = calculate_reward(next_all_game_info, former_all_game_info, next_pill_list, next_superpill_list, former_pill_list, former_superpill_list)
                ...
                # 监控训练过程: 初始loss为none，epsilon不下降是正常的，因为replaybuffer的batchsize是64，
                # 刚开始的64步只store，不update（update进去就直接return了）
                if loss is not None:
...
if __name__ == "__main__":
    main('ALE/MsPacman-v5', episodes=2)#Ms
