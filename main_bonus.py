...
class MockArgs:
    def __init__(self):
        ...
        self.game_name='ALE/MontezumaRevenge-v5'#'ALE/MsPacman-v5'#'ALE/MsPacmanNoFrameskip-v4'#'ALE/Pacman-v5'# 'ALE/MontezumaRevenge-v5'蒙特祖马
        ...
        self.mtzm_object_way=["接近梯子上缘时，可以顺着梯子向下走，反之可以顺着梯子往上走","接近绳子时，可以根据相对位置向右上方/左上方跳上绳子，或右下方/左下方跳下绳子","接近鬼时，若鬼与你同方向行进，就在距离鬼10点距离时起跳，若反方向行进，就在20点距离时起跳"]
        self.center_bone_history = (10000,10000)
args = MockArgs()
...
def run_code(num,env):
    ...
    for i in range(len(num)//2):
        print( num[2*i], num[2*i+1])
        observation, reward, terminated, truncated, info = single_action(env, num[2*i], num[2*i+1])
        if terminated or truncated:
            args.mtzm_object = {"ladder3，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True, "ladder2，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True, "ladder1，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True, "rope，接近绳子时，可以先第一步走到距离绳子小于25处，第二步再根据主人公和rope相对位置向右上方/左上方8帧跳上绳子，第三步再保持静止观察（也就是action0）10帧以后，第四步执行一遍与第二步相同的指令": True, "bone，bone在靠近你所以你先向左走7帧，然后向左上方起跳8帧即可": True, "key，靠近之后跳跃一下即可获取通关": True}
...
def find_nearest_active_item(head_pos, mtzm_dict, pos_list):
    ...
    # 定义物品索引映射
    item_index_map = {
        "key，靠近之后跳跃一下即可获取通关": 1,
        "bone，bone在靠近你所以你先向左走7帧，然后向左上方起跳8帧即可": 2,
        "rope，接近绳子时，可以先第一步走到距离绳子小于25处，第二步再根据主人公和rope相对位置向右上方/左上方8帧跳上绳子，第三步再保持静止观察（也就是action0）10帧以后，第四步执行一遍与第二步相同的指令": 3,
        "ladder1，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": {"，当前靠近顶部": 4, "，当前靠近底部": 5, "center": 6},
        "ladder2，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": {"，当前靠近顶部": 7, "，当前靠近底部": 8, "center": 9},
        "ladder3，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": {"，当前靠近顶部": 10, "，当前靠近底部": 11, "center": 12}
    }
    ...
    # 临时存储找到的最近物品的完整信息
    found_item_key = None  # 用于在字典中查找的键
    found_item_full_name = None  # 完整的物品名称（包含梯子部分）
    ...
    for item_name, is_active in mtzm_dict.items():
        ...
        if item_name.startswith("key")  or item_name.startswith("rope"):
            # 简单物品
            ...
            if item_pos is not None:
                ...
                if dist < min_distance:
                    ...
                    found_item_key = item_name
                    found_item_full_name = item_name
        elif item_name.startswith("bone"):
            # 敌人/骨头物品
            idx = item_index_map[item_name]
            item_pos = pos_list[idx]
            if item_pos is not None:
                ...
                if dist < min_distance:
                    min_distance = dist
                    nearest_item = f'{item_name},为了方便判断其行进方向，告知你上一次检测时bone位置是{args.center_bone_history}，而现在'
                    nearest_pos = item_pos
                    found_item_key = item_name
                    found_item_full_name = item_name
            ...
            for part, idx in ladder_data.items():
                ...
                if item_pos is not None:
                    ...
                    if dist < min_distance:
                        min_distance = dist
                        nearest_item = f"{item_name}{part}"
                        nearest_pos = item_pos
                        found_item_key = item_name  # 梯子本身的键
                        found_item_full_name = f"{item_name}{part}"  # 包含部分的完整名称
    
    # 只有在找到物品时才将其标记为已处理
    if found_item_key is not None and found_item_key in mtzm_dict:
        mtzm_dict[found_item_key] = False
        print(f"物品 '{found_item_full_name}' 已被标记为已处理 (is_active=False)")
...
def main(env_name, render=True, episodes=2):
    if env_name == "ALE/Pacman-v5" or env_name == "ALE/MsPacman-v5":
        ...
        observation,_,terminated,truncated,_ = single_action(env, 0, 1) 
        ...
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
            ...
            print("bone head", center_bone, center_head)
            dx = center_head[0] - center_bone[0]  # 1
            dy = center_head[1] - center_bone[1]  # 1
            ...
            nearest_item_name, nearest_item_pos, nearest_dist = find_nearest_active_item(
                center_head, args.mtzm_object, pos_information
            )
            ...
            print("分析")
            """重要的事情："""
            prompt=f"你身处游戏蒙特祖马第一关，需要找到去拿到钥匙的路线，碰到敌人以及角色走到没路的地方会摔落造成扣血。\
                                  图片尺寸是（160，210，3），图片左上角是(0,0),右下角是（160，210）。你目前的信息有：{all_information}。你的位置是{center_head}。请输出类似observation = single_action(env, Int(指令种类), Int(持续帧数)，持续帧数=距离除以速度)的指令，主人公速度为每帧可移动3像素,\
                                  你的可执行操作是————0：保持静止观察，1：跳跃，2：顺着梯子往上爬，3：右，4：左，5：\
                                  顺着梯子往下爬，6：右上，7：左上，8：右下，9：左下，10：向上跳，11：向右跳，12：向右跳，13：向下跳， \
                                14：右上且跳，15：左上且跳，16：右下且跳，17：左下且跳，输出指令种类和时间这两个数字组成的代码即可，请一次性输出完与该物品交互的所有指令！无需其他输出，其他输出会严重影响游戏"
            print(prompt)
            ...
            if terminated or truncated:
                args.mtzm_object = {"ladder3，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True, "ladder2，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True, "ladder1，第一步如果x轴距离大于5，就先左右移动到与梯子对齐，如果已经x距离小于5，就直接进行第二步，第二步接近梯子顶部时，可以顺着梯子向下走，反之靠近底部可以顺着梯子往上走，梯子总长33像素": True,  "rope，接近绳子时，可以先第一步走到距离绳子小于25处，第二步再根据主人公和rope相对位置向右上方/左上方8帧跳上绳子，第三步再保持静止观察（也就是action0）10帧以后，第四步执行一遍与第二步相同的指令": True, "bone，bone在靠近你所以你先向左走7帧，然后向左上方起跳8帧即可": True, "key，靠近之后跳跃一下即可获取通关": True}
            args.center_bone_history=center_bone
        while reward==0:
            frame+=1
            # 使用临时文件保存图像（避免污染项目目录）
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
            ...
            # 添加最近物品信息（如果找到了）
            if nearest_item_name and nearest_item_pos:
                all_information.append(f"距离最近的物品是{nearest_item_name}，位置是{nearest_item_pos}")

                # print(distance)
                cv2.imwrite('figure/A_'+str(frame)+".png", cv2.cvtColor(marked_img, cv2.COLOR_RGB2BGR))
                print("分析")
                """重要的事情："""
                prompt=f"图片左上角是(0,0),右下角是（160，210）。你目前的信息有：{all_information}。你的位置是{center_head}。请你进行反思与学习请输出类似observation = single_action(env, Int(指令种类), Int(持续帧数)，持续帧数=距离除以速度)的指令，主人公速度为每帧可移动3像素,\
                                    你的可执行操作是————0：保持静止观察，1：跳跃，2：顺着梯子往上爬，3：右，4：左，5：\
                                    顺着梯子往下爬，6：右上，7：左上，8：右下，9：左下，10：向上跳，11：向右跳，12：向右跳，13：向下跳， \
                                    14：右上且跳，15：左上且跳，16：右下且跳，17：左下且跳，输出指令种类和时间这两个数字组成的代码即可，与x距离较远的ladder去交互可以输出至多2行代码，与rope,bone交互时可以输出至多四行代码，与bone交互时帧数计算公式为（当前主人公x轴坐标-bone的现在x轴坐标-预留的22像素）除以主人公速度3像素每秒，bone始终会靠近主人公！请一次性输出完与该物品交互的所有指令!无需其他输出!其他输出会严重影响游戏！"
                print(prompt)
                result = call_qwen_vl('figure/A_'+str(frame)+".png", prompt)
                print(result)
                observation, reward, terminated, truncated, info = run_code(extract_num(result),env)
